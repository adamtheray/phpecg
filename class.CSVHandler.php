...
// Standard User functions
	...
	function ReadCSV() {			//read data into this->ItemsList and return it in an array 
		...
		while ($DataLine = fgetcsv ($fp, 3000, $this->Separator)) {
			for($i=0;$i<count($this->HeaderData);$i++){
				$DataLine[$i]=str_replace("SEPARATOR",$this->Separator,$DataLine[$i]);
	...
	function getHeaders() {			//get column names and return them in an array
		$this->ReadCSV();
	...
	function deleteColumn($columName){
		$this->ReadCSV();
		for($i=0;$i<count($this->HeaderData);$i++){
			if($this->HeaderData[$i]==$columName){
				array_splice($this->HeaderData,$i,1);
				break;
			}
		}
		$this->writeData();
	...
	function WriteData() {		//writing contents of ItemList to Datafile
		...
		while(list($key,$val)=each($this->ItemsList)) {
			for($i=0;$i<count($this->HeaderData);$i++){
				...
				$val[$writekey]=str_replace($this->Separator,"SEPARATOR",$val[$writekey]);
